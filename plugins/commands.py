...
async def start(client, message):
    ...
    if pre == "all":
        try:
            # Reconstruct string, file_id originally was batch_{key}_{offset}
            # `data` is `all_batch_-100123-456_0`
            parts = data.split("_")
            offset = int(parts[-1])
            # Drop 'all' (0) and 'batch' (1), combine the rest for the key
        ...
        # Re-apply filters just like pm_filter.py
